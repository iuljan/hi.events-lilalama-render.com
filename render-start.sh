...
if [ -z "$RENDER_EXTERNAL_URL" ]; then
    ...
    export VITE_API_URL_CLIENT="${VITE_FRONTEND_URL:-$RENDER_EXTERNAL_URL}/api"
    export APP_CND_URL="${VITE_FRONTEND_URL:-$RENDER_EXTERNAL_URL}/storage"
fi
export VITE_API_URL_SERVER=http://localhost:80/api
export VITE_FRONTEND_URL=${VITE_FRONTEND_URL:-${RENDER_EXTERNAL_URL:-"/"}}
export VITE_STRIPE_PUBLISHABLE_KEY=${VITE_STRIPE_PUBLISHABLE_KEY:-"pk_test-change-me"}
